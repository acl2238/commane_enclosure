...
carbon_monoxide_input = AnalogIn(ads, ADS.P0, ADS.P1)
...
destination = '/home/acl2238/scripts/k30/data/' # This is the specific path to the specific MSD on this raspb
# To change destination, change this string to the filepath to any drive/directory
os.chdir(destination)

# Designates which pin the GPIO is on
BUTTON_GPIO = 16 # check setmode
...
def button_callback(channel):
    if not GPIO.input(BUTTON_GPIO):
        ...
        if os.stat(outputfilename).st_size == 0: # If the output is empty, initial line with labels will be written
            output.write("Date, Time, Lattitude, Longitude, CO2 Value, CO Voltage, CO Value\n")
        ...
        for line in gps: 
            ...
            if "RMC" in line:
                ...
                currline = "%s, %s, %s %s , %s %s , %s, %s, %s\n" % (date, curtime, lat, latsign, long, longsign, carbon_dioxide_value, carbon_monoxide_voltage, carbon_monoxide_value)
